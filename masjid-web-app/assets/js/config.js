 ...
 *   PWA builders (server/routes/pwa.js and pwa-builder.py) replace the
 *   _LOADED_CONFIG assignment below with a resolved JSON object containing
 *   masjid-specific settings.  When running from source (dev mode),
 *   _LOADED_CONFIG remains empty and the _DEFAULTS below are used.
 *
 *   This replaces the old regex-patching approach where builders had to
 *   match and rewrite individual JS literals inside MASJID_DATA().
 */

/**
 * Build-time injected configuration.
 * PWA builders replace this single line with the resolved masjid settings.
 * In dev/source mode this remains an empty object and defaults apply.
 * --- DO NOT place anything else on this line; builders match it exactly. ---
 */
const _LOADED_CONFIG = {};

/**
 * MASJID_DATA: Customizable masjid-specific information.
 * _DEFAULTS are defined inline; _LOADED_CONFIG overrides them at build time.
 */
const MASJID_DATA = () => {
    const _DEFAULTS = {
    ...
    // For top-level keys, _LOADED_CONFIG values take precedence.
    const config = { ..._DEFAULTS, ..._LOADED_CONFIG };

    // Remap flat social keys (social_facebook, etc.) from builder JSON to nested masjidSocialLinks.
    // Builders store social links as flat keys matching masjid-settings.json schema.
    const SOCIAL_KEYS = ['facebook', 'instagram', 'x', 'youtube', 'whatsapp', 'telegram'];
    const hasFlatSocial = SOCIAL_KEYS.some(k => (`social_${k}`) in _LOADED_CONFIG);
    if (hasFlatSocial) {
        config.masjidSocialLinks = {};
        for (const k of SOCIAL_KEYS) {
            config.masjidSocialLinks[k] = _LOADED_CONFIG[`social_${k}`] || '';
        }
    } else if (_LOADED_CONFIG.masjidSocialLinks) {
        // If the builder provided nested format directly, use it
        config.masjidSocialLinks = { ..._DEFAULTS.masjidSocialLinks, ..._LOADED_CONFIG.masjidSocialLinks };
    }

    return config;
};

// Cache the resolved config once — avoids re-executing MASJID_DATA() multiple times.
const _resolvedConfig = MASJID_DATA();

const APP_CONFIG = {
    ..._resolvedConfig,

    // static variables
    masjidLogoPath: `assets/images/logos/${_resolvedConfig.masjidLogo}`,
    masjidBuildingImagePath: `assets/images/backgrounds/${_resolvedConfig.masjidBuildingImage}`,
    ...
    // the need to redeploy the app. The fallback is assets/data/iqamah-settings.json
    ...
    // the need to redeploy the app. The fallback is assets/data/notifications.json
